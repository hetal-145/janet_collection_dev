...
<div class="wrapper wrapper-content animated fadeInRight">   
    <div class="row">
        <div class="col-lg-12">
		    ...
		    <a href="#" class="btn btn-primary pull-left importxl custombtn" data-toggle="modal" data-target=".mdl_importxl">Import CSV For Product Price</a>		    
		    <a href="product/export_xls" class="btn btn-info pull-left exportxls custombtn">Export CSV For Product Price</a>
		    
		    <!-- brand -->
		    <a href="#" class="btn btn-primary pull-left brand_importxl custombtn" data-toggle="modal" data-target=".mdl_brand_importxl">Import Brand CSV</a>
		    
		    <a href="product/brand_export_xls" class="btn btn-info pull-left exportxls custombtn" data-type="brand">Export/Download Brand CSV</a>
...
<!--                                            <div class="col-sm-4">
                    ...
                    <div class="modal-dialog modal-md">
                        <div class="modal-content">
                            <div class="modal-header">
                                ...
                                <h4 class="modal-title" id="myModalLabel">Import Product CSV </h4>
                            </div>
		...
		<!-- Brand CSV Import -->
                <div class="modal fade mdl_brand_importxl" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
                    <div class="modal-dialog modal-md">
                        <div class="modal-content">
                            <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                ...
                                <h4 class="modal-title" id="myModalLabel">Import Brand CSV </h4>
                            </div>

                            <div class="modal-body">
                                <form name="frm_import_brand_xls" class="frm_import_brand_xls form-horizontal" enctype="multipart/form-data">
                                    <div class="panel-body">
                                        ...
                                        <div class="row">
                                            <div class="col-sm-12">                            
                                                <div class="form-group">
                                                    <span class="help-block m-b-none"><b>Upload File</b></span>
                                                    <input type="file" class="form-control import_file_brand_nm" name="import_file_brand_nm">
                                                </div>
                                            </div>   
                                        </div>
					<!--<div id="progress-wrp2"><div class="progress-bar2"></div ><div class="status2">0%</div></div>-->
                            ...
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                                <a class="btn upload_brand_importxl btn-primary pull-right">Save</a>
                                <span class="success_msg pull-right" style="color:green; padding: 7px;"></span>
                            </div>
...
//                        window.location.reload();
	...
	//Upload xls
        $('.upload_brand_importxl').click(function (e)  {
            e.preventDefault();
	    ...
	    $(progress_bar_id + " .status").text("0%");
            ...
            $('.form-control').removeClass('input_error');
            var valid = true;
            var frm = $('form[name = "frm_import_brand_xls"]');
            
            $(this).html('Processing...');
            var data = new FormData(frm[0]);
            //console.log(data);
            $.ajax({
                url: "product/brand_import_xls",
                type: "post",
                data: data,
                contentType: false,
                cache: false,
                ...
                {
		    $('.mdl_brand_importxl').attr("disabled", false);
		    console.log(resp1);
                    if (resp1 == 'success') {
                        alert('Brands Uploaded Successfully.');
                        $('.mdl_brand_importxl').html('Save');
                        window.location.reload();
                    } 
                    else if(resp1 == '3'){
			...
			if(resp[0] == '2'){
			    ...
			    alert('Please add brand name in csv file for brand '+resp[1]);
			    $('.mdl_brand_importxl').html('Save');     
			 }
			 else if(resp[0] == '5'){
            ...
            $.ajax({
                ...
                { 
                    ...
                    if (resp == 'success') {
                       ...
                       alert('Please add product pack size in csv file.');
                       $('.upload_importxl').html('Save');     
                    }
                    else if(resp == '8'){
                       alert('Please add product volume in csv file.');
                       $('.upload_importxl').html('Save');     
                    }
                    else if(resp == '13'){
                       alert('Please add product actual price in csv file.');
                       $('.upload_importxl').html('Save');     
                    }
        ...
        $(document).on('click', '.edit_data', function () {
			...
			if(product.gallery_image != "") {
				...
				$("#gimgs").append('<br><a href="<?php echo PRODUCT_S3_PATH; ?>'+value+'" name="product_gallery_view" id="product_gallery_view_'+v1+'">'+value+'</a>&nbsp;&nbsp;<button type="button" class="btn btn-xs btn-danger del_gimg" data-id="'+value+'" id="del_gimg_'+v1+'"><i class="fa fa-close"></i></button>');
			    });
			}
