...
//	$this->m_product->get_products_by_seller($user_id);
	...
	$spreadsheet->getProperties()->setCreator('Janet-Collection')
	    ...
	    ->setTitle('Product Price List')
	    ->setSubject('Product Price List')
	    ->setDescription('Product Price List')    
	    ->setCategory('Product Price List');
...
//		print_r($products); exit;
	...
	$spreadsheet->getActiveSheet()->setCellValue('B1', 'Volume');
	$spreadsheet->getActiveSheet()->setCellValue('C1', 'Pack Size');
	$spreadsheet->getActiveSheet()->setCellValue('D1', 'Actual Price');
	$spreadsheet->getActiveSheet()->setCellValue('E1', 'Normal Sell Price');
	$spreadsheet->getActiveSheet()->setCellValue('F1', 'Normal Discount');
	$spreadsheet->getActiveSheet()->setCellValue('G1', 'Loyalty Club Sell Price');
	$spreadsheet->getActiveSheet()->setCellValue('H1', 'Loyalty Club Discount');
	$spreadsheet->getActiveSheet()->setCellValue('I1', 'VIP Club Sell Price');
	$spreadsheet->getActiveSheet()->setCellValue('J1', 'VIP Club Discount');
...
//	echo "<pre>"; print_r($products); exit;
	...
	// Rename worksheet
	$spreadsheet->getActiveSheet()->setTitle('Product Price List');
	...
	header('Content-Disposition: attachment;filename="product_price_' . date("Y_m_d_H_i") . '.xls"');
...
//        print_r($_FILES); exit;        
        if (isset($_FILES['import_file_brand_nm']['name']) && $_FILES['import_file_brand_nm']['name'] && ($_FILES['import_file_brand_nm']['size'] > 0)) {
            $filename = $_FILES['import_file_brand_nm']['tmp_name']; 
	    $inputFileType = 'Xls';
