                ...
                if (!empty($checkout["products"])) {
                    foreach ($checkout["products"] as $products) {
					    ...
					    <p class="desc mb-0">Sold By: <?php echo $products["seller"]["seller_name"]; ?> <?php if($products["seller"]["is_open"] == 1) { ?><strong style="color:green;">(Open)</strong> <?php } else { ?> <strong style="color:red;">(Closed)</strong> <?php } ?> </p>
                                            <p class="desc mb-0">Delivery Charges: <?php echo CURRENCY_CODE . $products["delivery_charges"]; ?></p>
                                        </div>
					</div></a>
                                </div>
					...
					<input class="form-control text-center volume_id" type="hidden" value="<?php echo $products["volume_id"]; ?>" />
                                ...
                                <div class="col-4 col-sm-4 col-md-2 col-lg-2 col-xl-2 pl-xl-0 pl-lg-0 align-self-center text-center">
                                    <h5 class="mb-0"><?php echo CURRENCY_CODE . $products["total"]; ?></h5>
                                </div>
		    ...
		    <label class="display_label">No Products In Cart</label>
                        ...
                        <div class="col-4 col-sm-4 col-lg-2 col-md-2 col-xl-2 text-right">
                            <h4 class="mb-0"><?php
                                if (!empty($checkout["total_amount"])) {
                                    echo CURRENCY_CODE . $checkout["total_amount"];
                                } else {
                                    echo 0;
                                }
                                ?></h4>
			    <input type="hidden" id="gross_amount" name="gross_amount" value="<?php if(isset($checkout["total_amount"])) { echo $checkout["total_amount"]; } else { echo "0"; } ?>" />
                        ...
                        <div class="col-4 col-sm-4 col-lg-2 col-md-2 col-xl-2 text-right">
                            <h4 class="mb-0"><?php
                                if (!empty($checkout["delivery_charge"])) {
                                    echo CURRENCY_CODE . $checkout["delivery_charge"];
                                } else {
                                    echo 0;
                                }
                                ?></h4>
                    ...
                    <div class="row mt-3">
                        ...
                        <div class="col-4 col-sm-4 col-lg-2 col-md-2 col-xl-2 text-right">
                            <h4 class="mb-0"><strong><?php echo CURRENCY_CODE; ?></strong><strong id="net_amt"><?php
				...
				<div class="row other_opt">
					...
					<div class="tab-content" id="myTabContent">
						...
						<?php if(!empty($slots["slots"])) { foreach($slots["slots"] as $slot_val) { ?>						
							...
							<div class="pretty p-icon p-round p-smooth mt-0">
							    <input data-to_be_delivered="<?php echo $slots["date"]; ?>" name="time_slot" class="time_slot_cls" type="radio" value="<?php echo $slot_val["slot"]; ?>" data-sflag="<?php echo $slot_val["flag"]; ?>" />
							    <div class="state p-success">
								<i class="icon mdi mdi-check"></i>
								<label> </label>
							    </div>
							...
							<div class="media-body ml-2 align-self-center">
							    <h6 class="mt-0 mb-0"><?php echo $slot_val["slot"]; ?></h6>
							</div>
						    </div>
						<?php } } ?>
					    </div>
...
$(document).ready(function(){
    ...
    $(".time_slot_cls").click(function(){
	var sflag = $(this).data('sflag');
//	alert(sflag);
	$("#modal_submit_reviews").find(".alert_msg").html("");	
	if(sflag == '1') {
	    $("a.modal_submit_reviewss").trigger("click");
	    $("#modal_submit_reviews").find(".alert_msg").html("You cannot purchased both products as one seller is closed for this particular slot. Select any option from below given option: <br><br> 1. Schedule both products for later. <br> 2. Select an alternative product [from a different shop]. <br> 3. Remove the product.");
	...
	if ($("input[name='repeat_order_on']").is(':checked')) {
	...
	if ($("input[name='wallet']").is(':checked')) {
	...
	if ($("input[name='is_repeat_order']").is(':checked')) {
	    var is_repeat_order = 1;
	}
	else {
	    var is_repeat_order = 0;
	}
	...
	if ($("input[name='select_add_gc']").is(':checked')) {
	    var card_id = $("input[name='select_add_gc']:checked").val();
	}
	else {
	    var card_id = "";
	}
	...
	if (valid) {
		...
		data: datastring+"&to_be_delivered_date="+to_be_delivered+"&repeat_order_on="+rep_del_opt+"&order_done_type="+order_done_type+"&wallet="+wallet+"&is_repeat_order="+is_repeat_order+"&is_pick_up="+0+"&send_as_gift="+send_as_gift+"&order_type="+order_type+"&online="+online+"&card_id="+card_id,
		type: 'post',
...
//	    console.log(qty);
	...
	if (valid) {
		...
		success: function (data) {
			...
			$('.error_shipping').html('');
			$('.success_shipping').html(rep.response_msg);
			$('.Add_new_address').hide();
			$('.address_card_select').show();			
		    } 
		    else if (rep.status == 'false') {
			$('.error_shipping').html(rep.response_msg);
