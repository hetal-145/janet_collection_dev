...
class APN {
    ...
    function __construct($config=[]) {
        ...
        if (isset($config['user_type']) && $config['user_type'] == 'driver') {
            if (!file_exists($this->_ci->config->item('PermissionFile_Driver', 'apn'))) {
                show_error("APN Failed to connect: APN Permission file not found");
            }
            $this->keyCertFilePath = $this->_ci->config->item('PermissionFile_Driver', 'apn');
        }
        else {
