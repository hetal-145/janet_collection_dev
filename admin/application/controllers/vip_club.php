...
class Vip_club extends CI_Controller {
	...
	$xcrud->column_callback('feature_img', 'list_img_function');
        ...
        $this->load->view('product_in_vip_club', $data);
        $this->load->view('footer');
    }

    public function brand_in_vip_club() {
	...
	$xcrud->columns('category_id, brand_logo, brand_code, brand_name, status');                
        ...
        $xcrud->search_columns('category_id,brand_code, brand_name');         
        $xcrud->unset_add();
        $xcrud->unset_edit();       
        $xcrud->unset_remove();
        $xcrud->unset_view();
    ...
    public function category_in_vip_club() {
	...
	$xcrud->column_callback('category_img', 'list_img_function');
        $xcrud->columns('category_img, category_code, category_name, status');                
        $xcrud->fields('category_code, category_name, category_img, status'); 
	...
	$xcrud->change_type('status', 'select', '', array('1' => 'Active', '0' => 'Inactive')); 
        ...
        $xcrud->search_columns('category_name,category_code');         
        $xcrud->unset_add();
        $xcrud->unset_edit();       
        $xcrud->unset_remove();
        $xcrud->unset_view();
